...
from policydoc.cli import _validate_path, console
...
def analyze(path, output, wiki):
    ...
    console.print(f"  📄 政策文件: {len(list(root.rglob('*.pdf')) + list(root.rglob('*.docx')) + list(root.rglob('*.doc')))}")
    ...
    if wiki:
        ...
        if wiki_info.get("available"):
            console.print(f"  ✅ Wiki 核心文件: {wiki_info['required_files']['found']}/{wiki_info['required_files']['total']}")
