...
ENTITY_TYPES = {
    ...
    "Interface", "Type", "File", "Package",
    # 文档分析层
    "Document", "Policy", "Regulation", "Standard",
    # 业务实体层
    "Organization", "Person", "Project", "Event",
    "Platform", "University", "Government_Agency",
    # 知识层
    "Concept", "Term", "Category", "Domain",
    # 溯源层
    "SourceFile", "Analyzer", "ProvenanceRecord",
}
...
class Entity:
    ...
    provenance: Optional[Provenance] = None           # 溯源信息
    ...
    def to_dict(self) -> dict:
        ...
        if self.provenance:
            ...
            d["provenance"] = {"source_file": self.source_path, "analyzer": self.source, "method": "unknown", "confidence": self.confidence}
        return d
...
class Relation:
    ...
    weight: float = 1.0
    provenance: Optional[Provenance] = None  # 关系级别的溯源
    ...
    def to_json_ld(self, graph_url: str = "") -> dict:
        ...
        return {
            "@context": {"schema": "https://schema.org/", "codeanalyze": f"{base}/ontology/"},
            "@id": f"{base}/relation/{self.source_id}--{self.type}--{self.target_id}",
            "@type": ["schema:Relationship", f"codeanalyze:{self.type}"],
            "schema:name": rel.get("label", self.type),
            "codeanalyze:domain": rel.get("domain", ""),
            "codeanalyze:confidence": self.confidence,
            "schema:about": {"@id": f"{base}/entity/{self.source_id}"},
            "schema:relatedTo": {"@id": f"{base}/entity/{self.target_id}"},
        }
...
class KnowledgeGraph:
    ...
    def add_entity(self, entity: Entity, source_file: str = ""):
        ...
        if src:
            ...
            if sf_id not in self.entities:
                self.entities[sf_id] = Entity(
                    id=sf_id, name=src.split("/")[-1], type="SourceFile",
                    source=entity.source, domain="溯源",
                    properties={"path": src},
            ...
            self.relations.append(Relation(
                source_id=entity.id, target_id=sf_id,
                type="EXTRACTED_FROM",
        ...
        for eid, entity in other.entities.items():
            if eid not in self.entities:
                ...
                if existing.source != entity.source and eid not in {r.source_id for r in self.relations}:
                    self.relations.append(Relation(
                        source_id=eid, target_id=eid,
                        type="SAME_AS", confidence="INFERRED",
    ...
    def to_json_ld(self, graph_url: str = "") -> dict:
        ...
        return {
            ...
            "@graph": (
                [e.to_json_ld(base) for e in self.entities.values()] +
                [r.to_json_ld(base) for r in self.relations]
            ),
        }
    ...
    def to_cypher(self, label_field: str = "name") -> str:
        ...
        for e in self.entities.values():
            ...
            lines.append(
                ...
                f"{e.type}.confidence = {e.confidence}, "
                f"{e.type}.properties = '{props_json}', "
                f"{e.type}.provenance = '{prov_json}'"
            )
        lines.append("")
        for r in self.relations:
            ...
            lines.append(
                ...
                f"MERGE (a)-[:{r.type} {{confidence: '{r.confidence}', weight: {r.weight}, {prov_str}}}]->(b)"
            )
        return "\n".join(lines)
