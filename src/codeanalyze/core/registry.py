...
_TOOL_SPECS: list[ToolSpec] = [
    ToolSpec("graphify", "Semantic code knowledge graph (Tree-sitter AST + LLM)",
             python_module="graphify", python_fallback="graphifyy"),
    ToolSpec("gitnexus", "Repo dependency graph & call-chain analysis (LadybugDB)",
             cli="gitnexus"),
    ToolSpec("serena", "Symbol-level code retrieval & editing (LSP-based MCP)",
             cli="serena", python_module="serena_agent", check_plugin_dir=True),
    ToolSpec("docling", "IBM document → structured data (PDF/Word/HTML)",
             python_module="docling"),
    ToolSpec("docling_graph", "IBM Docling → Pydantic → Knowledge Graph (NetworkX)",
             python_module="docling_graph"),
    ToolSpec("marker", "PDF → Markdown/JSON/HTML (high accuracy)",
             cli="marker_single", python_module="marker"),
    ToolSpec("unstructured", "Document chunking & partition (PDF/HTML/DOCX)",
             python_module="unstructured"),
    ToolSpec("mineru", "中文PDF→Markdown/JSON (OpenDataLab, 中文最优)",
             python_module="magic_pdf", cli="magic_pdf", module_then_cli=True),
    ToolSpec("ripgrep", "Ultra-fast code search (Rust, 10x grep)",
             cli="rg"),
    ToolSpec("code_review_graph", "Tree-sitter persistent KG (token compression)",
             cli="code-review-graph", python_module="code_review_graph"),
]
...
def _check_tool(spec: ToolSpec) -> ToolInfo:
    ...
    # 1. Module with fallback (graphify)
    ...
    if spec.check_plugin_dir and not info.available:
        ...
        if plugin_dir.joinpath("package.json").exists():
            return ToolInfo(
                ...
                available=True, version="plugin", path=str(plugin_dir),
            )
    return info
    ...
    for p in [os.path.expanduser("~/Workspace/deepwiki-open"),
        ...
        if Path(p).joinpath("docker-compose.yml").exists():
            return ToolInfo(
                ...
                available=True, version="local", path=p,
            )
    return None
