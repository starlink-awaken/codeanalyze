...
def policy_graph_to_kg(pg: PolicyGraph, source: str = "official") -> KnowledgeGraph:
    ...
    for doc in pg.documents:
        ...
        entity = Entity(
            id=eid, name=doc.title or doc.filename[:50],
            type="Document", source=source,
            domain="公文" if doc.level != "其他" else "文档",
            confidence=0.9 if doc.doc_number else 0.6,
            properties={
                "filename": doc.filename, "doc_number": doc.doc_number or "",
                "issuing_org": doc.issuing_org or "", "level": doc.level,
                "pub_date": doc.pub_date or "", "domain_name": doc.domain,
                "format": doc.path.suffix, "size_bytes": doc.byte_size,
            },
            provenance=Provenance(
                source_file=sf_path, analyzer=source,
        ...
        if doc.doc_number:
            ...
            kg.add_entity(Entity(
                id=num_id, name=doc.doc_number, type="Policy",
                source=source, domain="政策", confidence=0.95,
                provenance=Provenance(source_file=sf_path, analyzer=source, method="regex"),
            ), source_file=sf_path)
            kg.add_relation(Relation(
                source_id=eid, target_id=num_id, type="REFERENCES",
        ...
        if doc.issuing_org:
            ...
            if org_id not in kg.entities:
                kg.add_entity(Entity(
                    id=org_id, name=doc.issuing_org, type="Organization",
                    source=source, domain="组织", confidence=0.7,
                    provenance=Provenance(source_file=sf_path, analyzer=source, method="regex_org"),
                ), source_file=sf_path)
            kg.add_relation(Relation(
                source_id=eid, target_id=org_id, type="BELONGS_TO",
                confidence="EXTRACTED",
                provenance=Provenance(source_file=sf_path, analyzer=source, method="regex_org"),
            ))
    ...
    for level, docs in pg.level_groups.items():
        ...
        if level_id not in kg.entities:
            kg.add_entity(Entity(
                id=level_id, name=level, type="Category",
                source=source, domain="政策层级", confidence=1.0,
            ))
        for doc in docs:
            eid = f"doc-{doc.filename[:40]}"
            if eid in kg.entities:
                kg.add_relation(Relation(
                    source_id=eid, target_id=level_id, type="BELONGS_TO",
                    confidence="EXTRACTED",
                    provenance=Provenance(
                        source_file=str(doc.path), analyzer=source,
    ...
    for domain, docs in pg.domain_groups.items():
        ...
        if domain_id not in kg.entities:
            kg.add_entity(Entity(
                id=domain_id, name=domain, type="Domain",
                source=source, domain="业务领域", confidence=0.9,
            ))
        for doc in docs:
            eid = f"doc-{doc.filename[:40]}"
            if eid in kg.entities:
                kg.add_relation(Relation(
                    source_id=eid, target_id=domain_id, type="CONTAINS",
                    confidence="EXTRACTED",
                    provenance=Provenance(
                        source_file=str(doc.path), analyzer=source,
    ...
    for rel in pg.relationships:
        ...
        kg.add_relation(Relation(
            ...
            provenance=Provenance(
                analyzer="official", method="cross_doc_reference",
                confidence=0.5,
            ),
        ))
...
def merge_code_kg(kg: KnowledgeGraph, repo_path: str) -> KnowledgeGraph:
    ...
    for ent in g_result.get("entities", []):
        ...
        if eid not in kg.entities:
            ...
            kg.add_entity(Entity(
                id=eid, name=ent_name, type=ent_type,
                source="graphify", domain="代码",
                confidence=0.85,
                properties={"path": ent_path, "language": ent.get("properties", {}).get("language", "")},
                provenance=Provenance(
                    source_file=ent_path, analyzer="graphify",
    ...
    for rel in g_result.get("relations", []):
        ...
        kg.add_relation(Relation(
            ...
            provenance=Provenance(
                analyzer="graphify", method="ast_extraction",
            ),
        ))
