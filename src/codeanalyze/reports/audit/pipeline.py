...
from codeanalyze.reports.audit.checkers import _extract_docx, _extract_pdf, _read_wiki
...
def _check_policy_graph(root: Path, wiki: Path, report: AuditReport) -> None:
    ...
    if notice.exists():
        ...
        g1.add_check(f"287号文提取 ({doc_num})", "287" in (doc_num or ""), f"图谱中287号: {'✅' if '287号' in policy_txt else '❌'}")
...
def _check_digital_platform(root: Path, wiki: Path, report: AuditReport) -> None:
    ...
    if v6.exists():
        ...
        for term in ["四链融合", "死亡之谷", "红蓝三区", "业务飞轮", "CAS", "C-T-F"]:
            g2.add_check(f"概念覆盖: {term}", term in dp_all, f"方案={'✅' if term in t else '❌'} | wiki={'✅' if term in dp_all else '❌'}")
    report.add_group(g2)
...
def _check_org_chart(root: Path, wiki: Path, report: AuditReport) -> None:
    ...
    for org_pdf in org_files:
        ...
        roles = set(re.findall(r'([一-鿿]{2,6}(?:组(?!长)|组长|主任|区长|书记|部长|局长|经理|秘书|协调员|主席|院长|校长|社长|总监))', t))
        ...
        g3.add_check(f"人物: {len(people)}个 (匹配: {matched})", matched >= 1 or len(people) > 0,
...
def _check_platform_data(root: Path, wiki: Path, report: AuditReport) -> None:
    ...
    if merged.exists():
        ...
        if p_names:
            ...
            g4.add_check(f"平台覆盖率: {covered}/{len(p_names)}", covered >= 1 or len(p_names) > 0,
...
def run_audit(project_root: str) -> AuditReport:
    ...
    if not wiki.is_dir():
        report.add_group(AuditGroup(name="Wiki 结构", checks=[
            {"label": "_工作机制/wiki 目录存在", "passed": False, "detail": ""}
        ]))
        return report
