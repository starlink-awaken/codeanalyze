...
class TestPolicydocBasic:
    ...
    def test_help(self):
        rc = subprocess.run([POLICYDOC, "--help"], capture_output=True).returncode
        assert rc == 0

    @pytest.mark.skipif(not POLICYDOC, reason="policydoc CLI not found")
    def test_version(self):
        rc = subprocess.run([POLICYDOC, "--version"], capture_output=True).returncode
        assert rc == 0

    @pytest.mark.skipif(not POLICYDOC, reason="policydoc CLI not found")
    def test_commands_present(self):
        result = subprocess.run([POLICYDOC, "--help"], capture_output=True, text=True)
    ...
    def test_documents_help(self):
        rc = subprocess.run([POLICYDOC, "documents", "--help"], capture_output=True).returncode
        assert rc == 0

    @pytest.mark.skipif(not POLICYDOC, reason="policydoc CLI not found")
    def test_audit_help(self):
        rc = subprocess.run([POLICYDOC, "audit", "--help"], capture_output=True).returncode
        assert rc == 0
