...
def _make_test_kg() -> KnowledgeGraph:
    ...
    kg.add_entity(Entity(
        id="doc-test", name="TestDoc", type="Document",
        source="official", domain="文档", confidence=0.9,
        source_path="/path/to/test.pdf",
    ))
    kg.add_entity(Entity(
        id="docnum-〔2026〕1号", name="〔2026〕1号", type="Policy",
        source="official", domain="政策", confidence=0.95,
    ))
    kg.add_entity(Entity(
        id="org-test", name="TestOrg", type="Organization",
        source="official", domain="组织", confidence=0.8,
    ))
    kg.add_relation(Relation(
        source_id="doc-test", target_id="docnum-〔2026〕1号",
        type="REFERENCES",
    ))
    kg.add_relation(Relation(
        source_id="doc-test", target_id="org-test",
        type="BELONGS_TO",
    ))
    return kg
