...
方案：
- ✅ CI 每周巡检已创建：`.github/workflows/tool-audit.yml`（每周一 09:00 UTC）
- 待办：对比每周版本快照，差异时自动开 issue
...
方案：
- ✅ 已标记 `@deprecated` + `warnings.warn()`（保留文件不破坏包结构）
- 待决策：删除或真实迁移。建议在 monorepo 阶段统一处理
...
方案：
- ✅ yamllint pre-commit hook 已添加
- ✅ markdownlint pre-commit hook 已添加
- 待办：跨文档引用完整性检查、governance-config.yaml 门禁检查器
...
方案：
- ✅ 架构 hook 添加 `stages: [commit, push]`
- 待办：CI governance step 运行 pre-commit run --all-files
